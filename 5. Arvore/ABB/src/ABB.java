...
public class ABB<K, V> implements IMapeamento<K, V> {
    ...
    private V pesquisar(No<K, V> raizArvore, K procurado) {
    	...
    	if (raizArvore == null) {
            ...
            throw new NoSuchElementException("Elemento não existe na árvore.");
    	}
        if(tamanho == 0) throw new IllegalStateException("Árvore vazia.");
     ...
     * retorna a qnt dos elementos na arvore
	...
	public int inserir(K chave, V item) {
        ...
        this.raiz = inserir(raiz, chave, item);
        return tamanho;
	}
    ...
    protected No<K, V> inserir(No<K, V> raizArvore, K chave, V item) {
        ...
        if (raizArvore == null) {
        	raizArvore = new No<K, V>(chave , item );
        } else {
        	...
        	if (comparacao < 0) {
                ...
                throw new IllegalStateException("A posição está ocupada.");
        	}
        }
        tamanho++;
